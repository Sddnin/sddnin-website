...
const GeminiChat = (() => {
  ...
  async function handleSend() {
    ...
    if (!AISettings.hasApiKey()) {
      appendMessage('error', '⚠️ Bạn chưa nhập API key Gemini. Nhấn biểu tượng 🔑 ở góc trên để cài đặt.', false);
      scrollToBottom();
      return;
    }
  ...
  async function sendToGemini(latestUserText) {
    ...
    // Xây contents từ lịch sử (đã bao gồm tin nhắn user vừa gửi ở cuối `history`)
