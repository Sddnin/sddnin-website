...
const GeminiChat = (() => {
  ...
  // 2 chế độ chat, MỖI chế độ có lịch sử hội thoại RIÊNG (localStorage
  // key khác nhau) — không dùng chung 1 history, vì đổi mode là đổi hẳn
  // persona (gia sư giải thích tiếng Việt <-> bạn chat chỉ nói tiếng Anh),
  // nếu gộp chung AI sẽ thấy ngữ cảnh lẫn lộn giữa 2 vai trò khác nhau.
  let currentMode = 'tutor'; // 'tutor' | 'buddy'
  function historyStorageKey() {
    return 'gemini_chat_history_' + currentMode;
  }
  ...
  let btnModeTutor, btnModeBuddy, chatTitleText, chatAvatar;
  ...
  function init() {
    ...
    btnModeTutor = document.getElementById('btn-chat-mode-tutor');
    btnModeBuddy = document.getElementById('btn-chat-mode-buddy');
    ...
    btnModeTutor.addEventListener('click', () => switchMode('tutor'));
    btnModeBuddy.addEventListener('click', () => switchMode('buddy'));
  ...
  // Chuyển chế độ chat: lưu lịch sử mode hiện tại lại, đổi mode, rồi
  // load + hiện lịch sử của mode mới (2 cuộc hội thoại độc lập hoàn toàn).
  function switchMode(mode) {
    if (mode === currentMode) return;
    currentMode = mode;
    btnModeTutor.classList.toggle('active', mode === 'tutor');
    btnModeBuddy.classList.toggle('active', mode === 'buddy');
    if (mode === 'tutor') {
      chatAvatar.textContent = '🤖';
      chatTitleText.textContent = 'Gia sư AI';
      inputEl.placeholder = 'Hỏi gia sư AI về từ vựng, ngữ pháp...';
    } else {
      chatAvatar.textContent = '🌎';
      chatTitleText.textContent = 'English Buddy';
      inputEl.placeholder = 'Type in English... (only English replies here!)';
    }
    loadHistory();
    renderHistory();
  }
  ...
  function clearConversation() {
    const label = currentMode === 'tutor' ? 'với gia sư AI' : 'với English Buddy';
    if (!confirm(`Xoá toàn bộ cuộc trò chuyện ${label}?`)) return;
    history = [];
    ...
    if (currentMode === 'buddy') {
      messagesEl.innerHTML = `
        <div class="chat-empty-state">
          ...
          Hey! I'm your English buddy. Let's chat — just type anything, and I'll only reply in English to help you practice!
          <div class="chat-suggestions">
            <button class="chat-suggestion-chip" data-q="Hey! How's your day going?">How's your day?</button>
            <button class="chat-suggestion-chip" data-q="What do you usually do on weekends?">Weekend plans</button>
            <button class="chat-suggestion-chip" data-q="Can you recommend a good movie?">Movie recs</button>
          </div>
        </div>
      `;
    } else {
  ...
  function buildSystemInstruction() {
    if (currentMode === 'buddy') {
      return {
        parts: [{
          text: 'You are a friendly native English speaker chatting casually with a Vietnamese learner who wants to practice conversational English. ' +
            'STRICT RULE: reply ONLY in English, never in Vietnamese, no matter what language the user writes in — this is for reflex/immersion practice. ' +
            'Keep your English simple and natural (everyday vocabulary, short-to-medium sentences) — this person is still learning, so avoid rare words or complex grammar that would intimidate them. ' +
            'Chat like a real friend: ask follow-up questions, share short opinions, keep the conversation flowing naturally. Do NOT lecture about grammar or vocabulary — that is a different mode of this app. ' +
            'If the user makes a noticeable grammar or word-choice mistake, do not point it out directly or explain the rule — just naturally reply using the corrected phrasing yourself, the way a native speaker would in casual conversation (implicit modeling, not correction).'
        }]
      };
    }
    return {
