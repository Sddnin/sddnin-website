const UI = (() => {
  ...
  const btnSkipCard = document.getElementById('btn-skip-card');
  ...
  function attachStudyEvents() {
    ...
    // Dùng chung cho mọi lối chuyển thẻ không qua swipe (nút bấm, phím
    // tắt, bắt đầu phiên mới) — đưa thẻ về mặt trước, xoá transform kéo
    // dở dang, ẩn gợi ý vuốt, rồi mới hiện nội dung thẻ đích.
    ...
    btnSkipCard.addEventListener('click', () => {
      // Bỏ qua KHÔNG rate — chỉ tiến con trỏ, không đụng level/nextReview
      // của thẻ đang xem, khác hẳn commitSwipe() vốn luôn rate trước.
      const next = Study.nextCard();
      if (next) {
        resetCardState();
        showCurrentStudyCard();
      } else {
    ...
    btnReshuffle.addEventListener('click', () => {
      Study.reshuffleRemaining();
      UI_Toast.show('Đã xáo lại thứ tự các thẻ còn lại', 'ok');
    });

    // Phím tắt: Space lật thẻ, ← thẻ trước, → bỏ qua. Chỉ hoạt động khi
    // study-area đang thực sự hiển thị, và bỏ qua nếu người dùng đang gõ
    // vào 1 ô input/textarea bất kỳ (kể cả input của các game khác) —
    // tránh phím Space/mũi tên bị nuốt mất khi họ đang gõ đáp án nơi khác.
    document.addEventListener('keydown', (e) => {
      ...
      if (e.code === 'Space') {
        e.preventDefault(); // tránh cuộn trang, hành vi mặc định của Space
        const flipping = !flashcard.classList.contains('flipped');
        flashcard.classList.toggle('flipped');
        swipeHint.style.display = flipping ? 'block' : 'none';
  ...
  function showCurrentStudyCard() {
    ...
    btnPrevCard.disabled = !Study.hasPrevCard();
    updateStudyProgress();
  }
